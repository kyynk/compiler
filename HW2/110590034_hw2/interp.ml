...
let is_false = function
  ...
  | Vbool false
  | Vstring ""
  | Vlist [||] -> true          (* Empty list is False *)
  | Vint n -> n = 0
  | _ -> false                  (* All other values are True *)
...
let rec compare_list a1 n1 a2 n2 i =
  if i = n1 && i = n2 then 0                   (* Lists are equal if all elements match *)
  else if i = n1 then -1                       (* First list is shorter *)
  else if i = n2 then 1                        (* Second list is shorter *)
  else let c = compare a1.(i) a2.(i) in        (* Compare elements *)
       if c <> 0 then c else compare_list a1 n1 a2 n2 (i + 1)

let rec compare_value v1 v2 = match v1, v2 with
  | Vlist a1, Vlist a2 ->                      (* Compare lists using compare_list *)
    compare_list a1 (Array.length a1) a2 (Array.length a2) 0
  | _ -> compare v1 v2                         (* Default comparison for other values *)
...
let binop op v1 v2 = match op, v1, v2 with
  ...
  | Blt, _, _ -> Vbool (compare_value v1 v2 < 0)           (* Less than *)
  | Ble, _, _ -> Vbool (compare_value v1 v2 <= 0)          (* Less than or equal *)
  | Bgt, _, _ -> Vbool (compare_value v1 v2 > 0)           (* Greater than *)
  | Bge, _, _ -> Vbool (compare_value v1 v2 >= 0)          (* Greater than or equal *)
...
let rec expr ctx = function
  ...
  | Ecst (Cstring s) ->
      Vstring s
  | Ebinop (Band, e1, e2) ->                     (* Logical AND operation *)
      let v1 = expr ctx e1 in
      if is_true v1 then expr ctx e2 else v1     (* Short-circuit evaluation *)
  | Ebinop (Bor, e1, e2) ->                      (* Logical OR operation *)
      let v1 = expr ctx e1 in
      if is_false v1 then expr ctx e2 else v1    (* Short-circuit evaluation *)
  | Ebinop (Badd | Bsub | Bmul | Bdiv | Bmod |   (* Binary arithmetic and comparison operators *)
            Beq | Bneq | Blt | Ble | Bgt | Bge as op, e1, e2) ->
      binop op (expr ctx e1) (expr ctx e2)       (* Perform binary operation *)
  | Eunop (Uneg, e1) ->                          (* Unary negation (e.g., -x) *)
      begin match expr ctx e1 with
        | Vint n -> Vint (-n)                    (* Negate integer *)
        | _ -> error "unsupported operand types" end
  | Eunop (Unot, e1) ->                          (* Logical NOT operation *)
      Vbool (is_false (expr ctx e1))             (* Invert the boolean value *)
  | Ecall ({id="len"}, [e1]) ->                  (* Call to built-in len function *)
      begin match expr ctx e1 with
        | Vstring s -> Vint (String.length s)    (* Length of a string *)
        | Vlist l -> Vint (Array.length l)       (* Length of a list *)
        | _ -> error "this value has no 'len'" end
  | Ecall ({id="list"}, [Ecall ({id="range"}, [e1])]) ->
      let n = expr_int ctx e1 in
      Vlist (Array.init (max 0 n) (fun i -> Vint i))  (* Generate a list from a range *)
  | Ecall ({id=f}, el) ->                        (* Function call *)
      if not (Hashtbl.mem functions f) then error ("unbound function " ^ f);
      let args, body = Hashtbl.find functions f in
      if List.length args <> List.length el then error "bad arity";  (* Check arity *)
      let ctx' = Hashtbl.create 16 in            (* Create new context for function *)
      List.iter2 (fun {id=x} e -> Hashtbl.add ctx' x (expr ctx e)) args el;
      begin try stmt ctx' body; Vnone with Return v -> v end  (* Execute function body *)
  | Elist el ->                                  (* List expression *)
      Vlist (Array.of_list (List.map (expr ctx) el)) (* Convert list of expressions to array *)
  | Eident {id} ->                               (* Find identifier *)
      if not (Hashtbl.mem ctx id) then error "unbound variable";  (* Check if the variable is in the context *)
      Hashtbl.find ctx id                        (* Retrieve the variable's value from the context *)
  | Eget (e1, e2) ->                             (* Get e2 from list *)
      begin match expr ctx e1 with
      | Vlist l ->                               (* Check if e1 evaluates to a list *)
          let i = expr_int ctx e2 in             (* Evaluate e2 to get the index, ensuring it's an integer *)
          (try l.(i) with Invalid_argument _ -> error "index out of bounds")  (* Access the list element, or raise an error if the index is out of bounds *)
      | _ -> error "list expected" end           (* Raise an error if e1 is not a list *)

(* Interpreting an expression, and checking that it is an integer *)

and expr_int ctx e = match expr ctx e with
  | Vbool false -> 0                             (* Convert boolean false to 0 *)
  | Vbool true -> 1                              (* Convert boolean true to 1 *)
  | Vint n -> n                                  (* If it's already an integer, return it *)
...
and stmt ctx = function
  ...
  | Sassign ({id}, e1) ->                        (* Assignment *)
      Hashtbl.replace ctx id (expr ctx e1)
  | Sset (e1, e2, e3) ->                         (* Set e2 in list e1 to e3 *)
      begin match expr ctx e1 with
...
and block ctx = function
  ...
  | s :: sl -> stmt ctx s; block ctx sl          (* Execute the first statement, then the rest of the block *)

(* Interpreting a file *)

let file (fl, s) =
  List.iter
    (fun (f,args,body) -> Hashtbl.add functions f.id (args, body)) fl;  (* Register all functions in the file *)
  stmt (Hashtbl.create 16) s                    (* Create a new context for global variables and execute the main statement block *)
